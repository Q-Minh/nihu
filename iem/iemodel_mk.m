function [M, K, iM, iK, iC, DOF] = iemodel_mk(model)
...
for iElem = 1 : nElem
    ...
    switch elem(2)
        ...
        case {111,122,133,134}
            ...
            map = [model.Nodes(elem(5:5+np-1),2:2+dim-1);model.MapNodes(elem(5+np:5+2*np-1),2:2+dim-1)];
...
M = sparse(I, J, M, nNodes, nNodes);
K = sparse(I, J, K, nNodes, nNodes);

iM = sparse(iI, iJ, iM, nNodes, nNodes);
iK = sparse(iI, iJ, iK, nNodes, nNodes);
iC = sparse(iI, iJ, iC, nNodes, nNodes);
