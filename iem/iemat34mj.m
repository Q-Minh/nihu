function [Ke,Ce,Me] = iemat34mj(M,P,ngauss,alpha,beta)
...
for c1 = 1:ngauss(1)
    for c2 = 1:ngauss(2)
        for c3 = 1:ngauss(3)
            ...
            for c4 = 1:l
                % Shape functions
                N(4*(c4-1)+1) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1-t)*(1-u);
                N(4*(c4-1)+2) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1+t)*(1-u);
                N(4*(c4-1)+3) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1+t)*(1+u);
                N(4*(c4-1)+4) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1-t)*(1+u);
                % Calculate gradients
                % d/ds
                gN(1,4*(c4-1)+1) = 1/8*(1-t)*(1-u)*((-1)*jacpoly(c4-1,alpha,beta,s)+(1-s)*djacpoly(c4-1,alpha,beta,s,1));
                gN(1,4*(c4-1)+2) = 1/8*(1+t)*(1-u)*((-1)*jacpoly(c4-1,alpha,beta,s)+(1-s)*djacpoly(c4-1,alpha,beta,s,1));
                gN(1,4*(c4-1)+3) = 1/8*(1+t)*(1+u)*((-1)*jacpoly(c4-1,alpha,beta,s)+(1-s)*djacpoly(c4-1,alpha,beta,s,1));
                gN(1,4*(c4-1)+4) = 1/8*(1-t)*(1+u)*((-1)*jacpoly(c4-1,alpha,beta,s)+(1-s)*djacpoly(c4-1,alpha,beta,s,1));
                % d/dt
                gN(2,4*(c4-1)+1) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1-u)*(-1);
                gN(2,4*(c4-1)+2) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1-u)*( 1);
                gN(2,4*(c4-1)+3) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1+u)*( 1);
                gN(2,4*(c4-1)+4) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1+u)*(-1);
                % d/du
                gN(3,4*(c4-1)+1) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1-t)*(-1);
                gN(3,4*(c4-1)+2) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1+t)*(-1);
                gN(3,4*(c4-1)+3) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1+t)*( 1);
                gN(3,4*(c4-1)+4) = 1/8*(1-s)*jacpoly(c4-1,alpha,beta,s)*(1-t)*( 1);
            end
            %Weighting function
