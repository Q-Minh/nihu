function [nodes, elements] = extract_bem_mesh(model)
%EXTRACT_BEM_MESH Extract bem mesh from a NiHu mesh structure
%  [NODES, ELEMENTS, NODIND, ELIND] = EXTRACT_BEM_MESH(MESH) extracts the
%  bem model fields from the NiHu structure MESH. The new
...
if ~isempty(ind3)
    elements(ind3,1) = 3;
    elements(ind3,2:4) = elem(ind3,5:7)-1; % C-style indexing
end
if ~isempty(ind4)
    elements(ind4,1) = 4;
    elements(ind4,2:5) = elem(ind4,5:8)-1; % C-style indexing
end
