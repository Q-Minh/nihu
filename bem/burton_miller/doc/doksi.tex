...
\end{align}
%
in the diagonals can be expressed using constant ($N(\bm{\xi}) = 1$) elements as:
%
\begin{align}
	\bG_{ii}
	& = \limeps \int_{\Gamma_\epsilon} G_k(\sx_i,\sy) \td \Gamma(\sy) +
	\limeps \int_{S_\epsilon} G_k(\sx_i, \sy) \td \Gamma(\sy) \nonumber \\
	& + \alpha \limeps \int_{\Gamma_\epsilon} \frac{\partial{G}_k (\sx_i, \sy)}{\partial \sn_x} \td \Gamma(\sy)
	+ \alpha \limeps \int_{S_\epsilon} \frac{\partial{G}_k (\sx_i, \sy)}{\partial \sn_x} \td \Gamma(\sy)
\end{align}

\begin{align}
	\bH_{ii}
	& = \limeps \int_{\Gamma_\epsilon}\frac{\partial{G}_k (\sx_i, \sy)}{\partial \sn_y} \td \Gamma(\sy)
    + \limeps \int_{S_\epsilon}\frac{\partial{G}_k (\sx_i, \sy)}{\partial \sn_y} \td \Gamma(\sy) \nonumber \\
	& + \alpha \limeps \int_{\Gamma_\epsilon} \frac{\partial^2{G}_k (\sx_i, \sy)}{\partial \sn_y \partial \sn_x} \td \Gamma(\sy) 
    + \alpha \limeps \int_{\S_\epsilon} \frac{\partial^2{G}_k (\sx_i, \sy)}{\partial \sn_y \partial \sn_x} \td \Gamma(\sy) \nonumber \\
\end{align}
...
\begin{align}
	G_1 = \limeps \int_{\Gamma_\epsilon} G_k(\sx_i,\sy) \td \Gamma(\sy) 
	& = \limeps \summ \intme \frac{\te ^{- \ti k r}}{4 \pi r} r \drph \nonumber \\
	& = \summ \intmn \frac{\te^{-\ti kr}}{4 \pi} \drph 
	= \summ \intms \frac{i}{4\pi k}\left(\te^{-\ti k R(\theta)} -1\right) \mathrm{d} \theta \nonumber \\
	& = \frac{i}{2k}\left(\frac{1}{2\pi}\summ \intms \te ^{-\ti kR(\theta)} \mathrm{d} \theta -1 \right)
\end{align}
...
\begin{align}
	H_1 = \limeps \int_{\Gamma_\epsilon}\frac{\partial{G}_k (\sx_i, \sy)}{\partial \sn_y} \td \Gamma(\sy) =
	\limeps \int_{\Gamma_\epsilon}-\frac{\te ^{-\ti k r}}{4 \pi r^2}(1 + \ti kr) \frac{\partial r}{\partial \sn_y} \mathrm{d} \Gamma_y = 0
\end{align}


Introducing the coordinate system $\tau(\sx)$, $s(\sx)$ and $n(\sx)$ over the Hemisphere, the second integral on the RHS can be expressed as:
...
The first integral on the RHS vanishes as $\epsilon \rightarrow 0$ and the second integral is constant (equals $2\pi$), therfore
...
\begin{align}
	G_3 = \limeps \int_{\Gamma_\epsilon} \frac{\partial{G}_k (\sx_i, \sy)}{\partial \sn_x} \td \Gamma(\sy) = 
	\int_{\Gamma_\epsilon} \frac{\te ^{-\ti kr}}{4 \pi r^2}(1 + \ti kr) \frac{\partial r}{\partial \sn_x} \mathrm{d} \Gamma_y = 0
\end{align}
...
\begin{align}
	G_4 & = \frac{1}{4\pi}\frac{2\pi}{3}\frac{\partial p(\sx)}{\partial n} = \frac{1}{6} \frac{\partial p(\sx)}{\partial n}
\end{align}

The third term in matrix $\bH$ can be integrated by making use of $\sn(\sx) = \sn(\sy)$ and $\partial r / \partial \sn_y = 0$ on $\Gamma_\epsilon$ and the double derivative of the Green function can be simplified as:
...
On the hemisphere surface $\partial r / \partial n(\sx) = -\cos(\theta)$, $\partial r / \partial n(\sy) = 1$ and $r = \epsilon$:
