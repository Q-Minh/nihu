...
[Hnf, Gnf] = bemHG_bm(mesh, k, alpha, [i j]);
...
Gq = Gnf * qs + ...
    mpcont_Gq_bm(rr, rs, nr, qs(gind).*w, tr, I, k, alpha, fs(gind), fr) +...
    (alpha/2 - 1i/(2*k)) * qs;
...
tol = 1e-4; % prescribed backward relative tolerance
[ps, eps] = my_gmres(...
    @(p) gmres_iter_neumann_bm(p, Hnf, rr, rs, nr, ns, w, gind, tr, I, k, alpha, fs, fr),...
