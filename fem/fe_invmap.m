...
for n = [2 3 4 6 8]
    ...
    if ~isempty(q)
        e = elements(q, 4+(1:n));
        bb(q,:) = [min(x(e),[],2)-tol, min(y(e),[],2)-tol, min(z(e),[],2)-tol, ...
            max(x(e),[],2)+tol, max(y(e),[],2)+tol, max(z(e),[],2)+tol];
    end
...
for iPoint = 1 : nPoint
    ...
    xf = points(iPoint,1:3); % field point coord
