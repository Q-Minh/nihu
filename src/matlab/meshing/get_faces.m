...
line = elements(elements(:,2) == ShapeSet.LinearLine.Id,[1 (5:6)]);
tria = elements(elements(:,2) == ShapeSet.LinearTria.Id,[1 (5:7)]);
%TODO: check compatibility with infinite elements
quad = elements((elements(:,2) == ShapeSet.LinearQuad.Id) | (elements(:,2) == 122),[1 (5:8)]);
tetra = elements(elements(:,2) == ShapeSet.LinearTetra.Id,[1 (5:8)]);
penta = elements((elements(:,2) == ShapeSet.LinearPenta.Id) | (elements(:,2) == 133),[1 (5:10)]);
hexa = elements((elements(:,2) == ShapeSet.LinearHexa.Id) | (elements(:,2) == 134),[1 (5:12)]);
...
faces(faces(:,6) ~= 0, 2) = ShapeSet.LinearQuad.Id;
faces(faces(:,6) == 0, 2) = ShapeSet.LinearTria.Id;
faces(faces(:,5) == 0, 2) = ShapeSet.LinearLine.Id;
faces(faces(:,4) == 0, 2) = 1;
