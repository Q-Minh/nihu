...
int main(void)
{
	...
	std::for_each (
		node,
		node+sizeof(node)/sizeof(node[0]),
		[&] (double n[]) { mesh.add_node(n); }
	);
	...
	std::for_each(
		...
		[] (TriaElem &e) { std::cout << e.getNodes().transpose() << std::endl; }
	);
	...
	std::for_each(
		...
		[] (LinQuadElem &e) { std::cout << e.getNodes().transpose() << std::endl; }
	);
	...
	std::for_each(
		...
		[] (PlaneQuadElem &e) { std::cout << e.getNodes().transpose() << std::endl; }
	);
	...
	std::for_each(
		...
		[] (QuadElem &e) { std::cout << e.getNodes().transpose() << std::endl; }
	);
