...
/**
 * \todo Jacobian is computed twice now. Find a good solution to this sickness
 */
template <class Space>
class location_with_normal :
...
{
public:
	typedef kernel_input_base<location_with_normal<Space> > crtp_base_t;
	typedef location<Space> base_t;

	/** \todo this explicit typedef is needed here because of the diamond */
	typedef typename base_t::space_t space_t;
	typedef typename base_t::x_t x_t;

	template <class elem_t, class quadrature_elem_t>
	location_with_normal(elem_t const &elem, quadrature_elem_t const &q)
		: crtp_base_t(elem, q), base_t(elem, q)
	{
