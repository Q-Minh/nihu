...
struct MyKernel
{
	...
	return_type operator()(location_input_2d const &x, location_input_2d const &y, wave_number_data<std::complex<double> > const &d)
	{
...
struct kernel_traits<MyVeryKernel>
{
	...
	typedef wave_number_data<std::complex<double> > data_t;
...
{
public:
	MyVeryKernel(std::complex<double> s) : kernel_base<MyVeryKernel>(data_t(s)) {}
};
...
int main(void)
{
	...
	auto G_op = create_integral_operator(MyVeryKernel(k));
