...
//! [Surface mesh]
	...
	auto surface = create_mesh(surf_nodes, surf_elements, _quad_1_tag());
//! [Surface mesh]

//! [Field point mesh]
	auto field_nodes = dMatrix(rhs[2]);
	auto field_elem = dMatrix(rhs[3]);
	auto field = create_mesh(field_nodes, field_elements, _quad_1_tag());
//! [Field point mesh]
...
//! [Kernel and weighted residual]
	double wave_number = *mxRealData(rhs[4]);
