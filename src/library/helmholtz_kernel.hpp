...
public:
	...
	*/
	void set_k(wave_number_type const &k)
	{
		m_k = k;
	}
	...
	*/
	wave_number_type const &get_k(void) const
	{
		return m_k;
	}
...
class helmholtz_G_kernel :
	...
	public kernel_with_wave_number<std::complex<scalar_t> >
{
public:
	...
	*/
	result_t operator()(test_input_t const &x, trial_input_t const &y) const
	{
		...
		return std::exp(-std::complex<scalar_t>(0.0,1.0)*get_k()*r) / r / (4.0 * M_PI);
	}
	...
	* \return the kernel value K(x,y)
	*/
	unsigned estimate_complexity(test_input_t const &x, trial_input_t const &y, scalar_t s) const
	{
...
public:
	...
	*/
	result_t operator()(test_input_t const &x, trial_input_t const &y) const
	{
		...
		scalar_t r2 = rvec.squaredNorm();
		scalar_t r = sqrt(r2);
		std::complex<scalar_t> ikr(std::complex<scalar_t>(0.,1.)*get_k()*r);
