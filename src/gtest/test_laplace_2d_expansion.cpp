...
{
	typedef NiHu::fmm::laplace_2d_fmm fmm_t;
	...
	double eps_p2m2p = std::abs(p2p - (m2p * p2m)(0).real()) / std::abs(p2p);
	EXPECT_LE(eps_p2m2p, eps_req);

	// test p2l and l2p (local expansion)
	double eps_p2l2p = std::abs(p2p - (l2p * p2l)(0).real()) / std::abs(p2p);
